...
#Tables Use case 1: Branch, Employees, Customers and Accounts
...
INSERT INTO CUSTOMER (CUSTID, FNAME, MNAME, LNAME, OCCUPATION, DOB)
...
INSERT INTO ACCOUNT (ACNUMBER, CUSTID, BID, CURBAL, OPNDT, ATYPE, ASTATUS)
...
INSERT INTO EMPLOYEE (EMP_NO, BRANCH_NO, FNAME, MNAME, LNAME, DEPT, DESIG, MNGR_NO)
...
SELECT *
FROM (
   SELECT CUSTID,CURBAL,row_number() OVER ( ORDER BY CURBAL asc) AS rn
   FROM   account
   )sub
WHERE  rn = 1;
...
select concat(FNAME,LNAME) as C_Name,DESIG 
from employee 
where DESIG='Manager';

#13. List name of all employees whose name ends with a
select EMP_NO, FNAME as C_Name,DESIG 
from employee 
where FNAME LIKE '%a';
...
select c.CUSTID,c.FNAME as Customer_Firstname,a.ACNUMBER  
from customer c join account a
on c.CUSTID=a.CUSTID;

#16. Write a query to display the customer’s number, customer’s firstname, branch id 
#and balance amount for people using JOIN.
select e.EMP_NO, e.FNAME, e.BRANCH_NO, a.CURBAL
from employee e join account a
on e.EMP_NO=a.CUSTID;

#17. Create a virtual table to store the customers who are having the accounts 
#in the same city as they live
select * from branch;
select * from customer;
update customer set CITY='Mumbai';

CREATE VIEW cust_city AS(
select c.CUSTID,c.FNAME,c.CITY,b.NAME as Branch_City from customer c join account a
on c.CUSTID=a.CUSTID
join branch b
...
create table T_transaction(TID int auto_increment primary key not null, 
							Custid int,
                            Account_No int,
                            Bid int,
                            Amount int,
...
ADD constraint Custid_key
FOREIGN KEY (Custid) REFERENCES 
customer(CUSTID);
 
ALTER TABLE T_transaction
ADD constraint Account_No_key
FOREIGN KEY (Account_No) REFERENCES 
account(ACNUMBER);

ALTER TABLE T_transaction
ADD constraint Bid_key
...
DELIMITER $
CREATE TRIGGER fan AFTER INSERT ON t_transaction
FOR EACH ROW BEGIN
 UPDATE account SET account.CURBAL=account.CURBAL+NEW.Amount WHERE account.CUSTID=NEW.Custid and NEW.TOT='deposit';
 UPDATE account SET account.CURBAL=account.CURBAL-NEW.Amount WHERE account.CUSTID=NEW.Custid and NEW.TOT='withdraw';
END $
DELIMITER ;

drop trigger OnSalesInsert; 
drop trigger tran; 
select * from account;
select * from t_transaction;

insert into t_transaction values (11,1,1,1,1000,'deposit',current_date(),1);
insert into t_transaction values (12,2,2,2,1000,'withdraw',current_date(),1);
insert into t_transaction values (13,2,2,2,700,'deposit',current_date(),1);


#19. Write a query to display the details of customer with second highest balance 
SELECT *
FROM (
   SELECT *,dense_rank() OVER ( ORDER BY CURBAL desc) AS rn
   FROM   account
   )sub
WHERE  rn = 2;
...
#TO DISK = 'E:\Great Lakes\DBMs project';
...
#1. Click on “Data Export” under the “Management” tab on the left sidebar
#2. You will get a list of available databases.
#Click on the checkbox next to the database you want to export.
#3. Choose the appropriate option under “Export Options.”
#4. Click on “Start Export.”
#You can track the process through “Export Progress.”
...
select * from order_header where ORDER_ID=10012;
select * from order_items where PRODUCT_ID in (206,207,208,204);
select * from product where PRODUCT_CLASS_CODE=2051;
select * from product_class ;
select * from shipper;

#2. List the product description, class description and price of all products which are shipped. 
select p.PRODUCT_ID ,p.PRODUCT_DESC,p.PRODUCT_PRICE,pc.PRODUCT_CLASS_DESC,oh.ORDER_STATUS 
...
#Address(address_id)-Online customer(address_id,product_class_code)->
#->product(product_class_code)-product_class()
SELECT
    customer_id
...
WHERE
    ...
    AND customer_id NOT IN (
        SELECT customer_id
        FROM order_items JOIN order_header ON order_items.order_id = order_header.order_id
        WHERE product_id IN (
            SELECT product_id
            FROM product_class
